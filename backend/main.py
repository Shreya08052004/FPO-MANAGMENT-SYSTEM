...
def get_farmer(farmer_id: str):
    f = next((f for f in farmers if f["id"] == farmer_id), None)
    if not f:
        raise HTTPException(status_code=404, detail="Farmer not found")
    farmer_tx = [t for t in transactions if t["farmer_id"] == farmer_id]
    return {**f, "transactions": farmer_tx[:5], "total_transactions": len(farmer_tx)}
...
def get_transactions(page: int = 1, limit: int = 10, crop: Optional[str] = None, status: Optional[str] = None):
    result = transactions[:]
    if crop and crop != "All":
...
def village_stats():
    ...
    for f in farmers:
        v = f["village"]
        if v not in stats:
            stats[v] = {"village": v, "farmers": 0, "total_land": 0, "avg_income": 0, "incomes": []}
        stats[v]["farmers"] += 1
        stats[v]["total_land"] += f["land_acres"]
        stats[v]["incomes"].append(f["income"])
    result = []
    for v, s in stats.items():
        result.append({"village": v, "farmers": s["farmers"], "total_land": round(s["total_land"], 1), "avg_income": round(sum(s["incomes"])/len(s["incomes"]))})
    return {"data": sorted(result, key=lambda x: x["farmers"], reverse=True)[:10]}
...
def income_trend():
    ...
    for i in range(12):
        ...
        base = 180000 + random.randint(-20000, 40000)
        data.append({"month": month, "avg_income": base, "min_income": base - random.randint(10000, 40000), "max_income": base + random.randint(20000, 80000)})
